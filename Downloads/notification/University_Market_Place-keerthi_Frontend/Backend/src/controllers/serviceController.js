...
// ============================================================
// 🔔 NOTIFICATION IMPORT — added for notification manager
// ============================================================
...
export const createService = async (req, res) => {
  try {
    const doc = await Service.create({
      ...req.body,
      ownerId: req.userId || req.body.ownerId,
    });

    // ============================================================
    // 🔔 NOTIFICATION #1 — Service Created
    // Notifies the owner when their service is successfully created.
    // Channels: in-app only (owner just did the action, no need for email/push/sms)
    // ============================================================
    try {
      await notify({
        userId: doc.ownerId,
        ...
        body: `Your service "${doc.title}" has been created and is ready to publish.`,
        channels: ["in-app"],
        metadata: { serviceId: doc._id.toString() },
      });
    } catch (notifErr) {
      console.error("Notification failed (createService):", notifErr.message);
    }
    // ============================================================
...
export const getServiceById = async (req, res) => {
  try {
    ...
    if (!isOwner) {
      ...
      // ============================================================
      // 🔔 NOTIFICATION #2 — Service View Milestone
      // Notifies the owner when their service hits a view milestone
      // (every 10 views). Channels: in-app only (informational nudge).
      // ============================================================
      const VIEW_MILESTONE = 10;
      if (doc.viewCount % VIEW_MILESTONE === 0) {
        try {
          await notify({
            userId: doc.ownerId,
            type: "system",
            title: `Your service hit ${doc.viewCount} views! 👀`,
            body: `"${doc.title}" just reached ${doc.viewCount} views. Keep it up!`,
            channels: ["in-app"],
            metadata: { serviceId: doc._id.toString(), viewCount: String(doc.viewCount) },
          });
        } catch (notifErr) {
          console.error("Notification failed (viewMilestone):", notifErr.message);
        }
      }
      // ============================================================
    }
...
export const addServiceReview = async (req, res) => {
  try {
    ...
    // ============================================================
    // 🔔 NOTIFICATION #3 — New Review Received
    // Notifies the service owner when a student leaves a review.
    // Channels: in-app + email (important event, owner should know ASAP).
    // Also notifies the reviewer that their review was submitted.
    // ============================================================
    try {
      ...
      await notify({
        ...
        body: `${reviewerName} left a ${rating}-star review on "${doc.title}": "${comment.slice(0, 60)}${comment.length > 60 ? "..." : ""}"`,
      ...
      // Notify the reviewer (confirmation)
      await notify({
        userId: reviewerId,
        type: "system",
        title: "Review Submitted Successfully ✅",
        body: `Your ${rating}-star review on "${doc.title}" has been posted.`,
        channels: ["in-app"],
        metadata: { serviceId: doc._id.toString() },
      });
    } catch (notifErr) {
      console.error("Notification failed (addServiceReview):", notifErr.message);
    }
    // ============================================================
