...
const RatingReview = () => {
  ...
  const [editHoveredStar, setEditHoveredStar] = useState(null);
  const [reviews, setReviews] = useState([
    {
      id: 1,
      name: 'Emily Chen',
      avatar: '🦊',
      avatarBg: 'bg-gradient-to-br from-[#FF6B6B] to-[#FF8E53]',
      date: 'March 2, 2026',
      rating: 5,
      comment: 'Absolutely exceptional service! The platform exceeded my expectations with its intuitive design and seamless functionality. Highly recommended for anyone looking for quality.',
      verified: true,
      isOwn: false,
      likes: 24,
      liked: false,
      replies: [
        {
          ...
          name: 'Sarah Johnson',
          avatar: '🐼',
          avatarBg: 'bg-gradient-to-br from-[#4ECDC4] to-[#45B7D1]',
          date: 'March 3, 2026',
          comment: 'Totally agree! The platform is amazing.',
          verified: true
        }
      ],
      showReplyBox: false,
    ...
    },
    {
      ...
      avatar: '🦁',
      avatarBg: 'bg-gradient-to-br from-[#5D9B9B] to-[#4A7C7C]',
      date: 'March 1, 2026',
      rating: 5,
      comment: 'Game-changing experience. The attention to detail and professional approach made all the difference. Customer support is outstanding and always helpful.',
      verified: true,
      isOwn: false,
      likes: 18,
      liked: false,
    ...
    },
    {
      ...
      avatar: '🐨',
      avatarBg: 'bg-gradient-to-br from-[#A06AB4] to-[#C06BB4]',
      date: 'February 28, 2026',
      rating: 4,
      comment: 'Really great platform with tons of features. The interface is clean and modern. Would love to see more customization options in future updates.',
      verified: true,
      isOwn: false,
      likes: 12,
      liked: false,
    ...
    },
    {
      ...
      name: 'James Liu',
      avatar: '🐧',
      avatarBg: 'bg-gradient-to-br from-[#2C3E50] to-[#3498DB]',
      date: 'February 27, 2026',
      rating: 5,
      comment: 'Five stars all the way! The quality of service is consistent and reliable. I\'ve been using this for months and never had any issues.',
      verified: true,
      isOwn: false,
      likes: 42,
      liked: false,
  ...
  const averageRating = (Object.entries(ratings).reduce((acc, [stars, count]) => 
    acc + (parseInt(stars) * count), 0) / totalReviews).toFixed(1);
  ...
  // Handle edit save
  const handleEditSave = (reviewId, oldRating) => {
    if (editText.trim() && editRating > 0) {
      ...
      // Update ratings count if rating changed
      if (oldRating !== editRating) {
        setRatings(prev => ({
          ...prev,
          [oldRating]: prev[oldRating] - 1,
          [editRating]: prev[editRating] + 1
        }));
      }
  ...
  const handleDeleteReview = (reviewId, rating) => {
    ...
    return (
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg
            key={star}
            className={`${starSize} transition-all duration-300 ${interactive ? 'cursor-pointer hover:drop-shadow-glow' : 'cursor-default'} ${
              interactive && currentHoveredStar >= star ? 'scale-110 drop-shadow-glow' : ''
            }`}
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl font-medium transition-all duration-300 ${
                localEditText.trim() && localEditRating > 0
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#F8F9FF] via-[#F0F3FF] to-[#E9ECFF] py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      {editingReview && (
        <EditReviewModal
          ...
          onSave={(id, oldRating, newText, newRating) => {
            // Update review in list
            setReviews(reviews.map(review => 
              review.id === id 
                ? { 
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-4">
                      <button
                        ...
                        className={`text-sm transition-all duration-300 flex items-center gap-1 group/btn ${
                          review.liked ? 'text-[#FF6B6B]' : 'text-[#9CA3AF] hover:text-[#FF6B6B]'
                        }`}
                      >
                        <svg
                          className={`w-4 h-4 transition-all duration-300 group-hover/btn:scale-110 ${
                            review.liked ? 'fill-current animate-bounce-subtle' : 'fill-none'
                          }`}
