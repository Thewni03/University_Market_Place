...
export default function Profile() {
  ...
  const fetchOwnerRevenueAnalytics = async (mode) => {
    ...
    try {
      const monthParam =
        mode === "week" ? `&month=${encodeURIComponent(selectedRevenueMonth)}` : "";
      const response = await fetch(
        `${API_BASE_URL}/api/services/analytics/views?ownerId=${encodeURIComponent(
          profileUserId
        )}&mode=${encodeURIComponent(mode)}${monthParam}`
      ...
      const result = await response.json();
      if (!response.ok) {
        throw new Error(result.error || result.message || "Failed to load revenue analytics.");
      }
      setRevenueAnalytics({
        totalViews: Number(result?.data?.totalViews || 0),
        totalBookings: Number(result?.data?.totalBookings || 0),
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          {revenueLoading ? (
            ...
            <div>
              <div className="mb-4">
                <p className="text-4xl font-display font-bold text-foreground">
                  LKR {revenueAnalytics.totalRevenue.toLocaleString()}
                </p>
